        ...
        .subscribe(
          (response: ApiListResponse) => {
            this.data = response.data.map(item => ({
              ...
              'ID_importação': item.imp_id,
              'Placa do Carro': item.car_plate,
              'Lote': item.impd_lote,
              'Tranferencia UF ': item.impd_uf_transfer,
              'Data Inicio': item.impd_start_date,
              'Previsao de Conclusao ': item.impd_target_date,
              'Kit doc Completo': item.impd_complete_kit,
              'Restricoes': item.impd_restrition,
              'Debitos Veiculares': item.impd_debit,
              'Comunicacao de Venda': item.impd_sale_com,
              'Vistoria Veicular': item.impd_vistory_complete,
              'Pendencias Comprador': item.impd_saler_pending,
              'CRLV-e Disponível': item.impd_crlv_avaible,
              'Trocar Placa': item.impd_plate_change,
            }));
